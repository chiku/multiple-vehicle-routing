...
class Trip
  ...
  def equal?(other)
    self.eql?(other) or ( other.instance_of?(self.class) and (center == other.center) and cities_with_equivalent_order?(other) )
  end
  ...
  def total_load
    cities.inject(0){|sum, city| sum + city.capacity}
  end
end
