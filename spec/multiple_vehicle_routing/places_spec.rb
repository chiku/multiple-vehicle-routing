...
describe MultipleVehicleRouting::Places do
  ...
  let(:city_3)   { MultipleVehicleRouting::City.new(name: 'b', coordinates: MultipleVehicleRouting::Coordinates.new(0, 7), capacity: 5)     }
  ...
  describe 'at a specified location' do
    let(:places) { described_class.new(center_1, city_1) }
    ...
    it 'is nothing when out of range' do
      expect(places.at(2)).to(be_instance_of(MultipleVehicleRouting::BlankPlace))
    end
  end

  describe 'first place' do
    it 'is the place that occurs at the beginning' do
      expect(described_class.new(center_1, city_1).first_place).to(eq(center_1))
    end

    it 'is not present in empty places' do
      expect(described_class.new.first_place).to(be_instance_of(MultipleVehicleRouting::BlankPlace))
    end
  end

  describe 'last place' do
    it 'is the place that occurs at the end' do
      expect(described_class.new(center_1, city_1).last_place).to(eq(city_1))
    end

    it 'is not present in empty places' do
      expect(described_class.new.last_place).to(be_instance_of(MultipleVehicleRouting::BlankPlace))
    end
  end

  describe 'begins with center' do
    it 'is true when the first place is a center' do
      expect(described_class.new(center_1, city_1)).to(be_begins_with_center)
    end

    it 'is false when the first place is a city' do
      expect(described_class.new(city_1, center_1)).not_to(be_begins_with_center)
    end

    it 'is false when no places are present' do
      expect(described_class.new).not_to(be_begins_with_center)
    end
  end

  describe 'intermediates' do
    it 'gives the places except the extremes' do
      places = described_class.new(center_1, city_1, city_2, center_1)
      intermediate_places = described_class.new(city_1, city_2)
  ...
  describe 'has center' do
    it 'is true when a center is present' do
      expect(described_class.new(city_1, center_1)).to(have_center)
    end

    it 'is false when no centers are present' do
      expect(described_class.new(city_1, city_2)).not_to(have_center)
    end

    it 'is false when no places are present' do
      expect(described_class.new).not_to(have_center)
    end
  end

  describe 'ends with center' do
    it 'is true when the last place is a center' do
      expect(described_class.new(city_1, center_1)).to(be_ends_with_center)
    end

    it 'is false when the last place is a city' do
      expect(described_class.new(center_1, city_1)).not_to(be_ends_with_center)
    end

    it 'is false when no places are present' do
      expect(described_class.new).not_to(be_ends_with_center)
    end
  end

  describe 'ends with city' do
    it 'is true when the last place is a city' do
      expect(described_class.new(center_1, city_1)).to(be_ends_with_city)
    end

    it 'is false when the last place is a center' do
      expect(described_class.new(center_1, city_1, center_1)).not_to(be_ends_with_city)
    end

    it 'is false when no places are present' do
      expect(described_class.new).not_to(be_ends_with_city)
    end
  end

  describe 'has same center at extremes' do
    it 'is false when the last place is a city' do
      expect(described_class.new(center_1, city_1)).not_to(have_same_center_at_extremes)
    end

    it 'is false when the first place is a city' do
      expect(described_class.new(city_1, center_1)).not_to(have_same_center_at_extremes)
    end

    it 'is false when centers at beginning and end differ' do
      expect(described_class.new(center_1, city_1, center_2)).not_to(have_same_center_at_extremes)
    end

    it 'is true when centers at beginning and end are same' do
      expect(described_class.new(center_1, city_1, center_1)).to(have_same_center_at_extremes)
    end
  end

  describe 'has unique cities' do
    it 'is false when a city is present twice' do
      expect(described_class.new(city_1, city_1)).not_to(have_unique_cities)
    end

    it 'is true when all cities are unique' do
      expect(described_class.new(city_1, city_2)).to(have_unique_cities)
    end

    it 'is true when all cities are unique with repeated centers' do
      expect(described_class.new(center_1, city_1, city_2, center_1)).to(have_unique_cities)
    end
  end

  describe 'has unique places' do
    it 'is false when a city is present twice' do
      expect(described_class.new(center_1, city_1, city_1)).not_to(have_unique_places)
    end

    it 'is false when a center is present twice' do
      expect(described_class.new(center_1, city_1, center_1, city_2)).not_to(have_unique_places)
    end

    it 'is true when all places are unique' do
      expect(described_class.new(center_1, city_1, center_2, city_2)).to(have_unique_places)
    end
  end

  describe 'has no intermediate center' do
    it 'is true no centers are present in the middle' do
      expect(described_class.new(center_1, city_1, center_1)).to(have_no_intermediate_center)
    end

    it 'is false when a center is present in the middle' do
      expect(described_class.new(center_1, center_2, city_1, center_1)).not_to(have_no_intermediate_center)
    end
  end

  describe 'has no consecutive centers' do
    it 'is true when no centers are present together' do
      expect(described_class.new(center_1, center_2, city_1)).not_to(have_no_consecutive_center)
    end

    it 'is false when a center are separated by cities' do
      expect(described_class.new(center_1, city_1, center_2, city_2)).to(have_no_consecutive_center)
    end
  end

  describe 'add' do
    it 'builds up the places' do
      places = described_class.new
      places << center_1
      places << city_1
  ...
  describe 'to_s' do
    it 'serialize to a string' do
      expect(described_class.new(center_1, city_1, city_2, center_1).to_s).to(eq('(A -> a -> b -> A)'))
    end
  end

  describe 'equals' do
    let(:places) { described_class.new(center_1, city_1, city_2, center_1) }
    ...
    it 'another places with same order' do
      another_places = described_class.new(center_1, city_1, city_2, center_1)
  ...
  describe 'reverse' do
    it 'is the list of places read backwards' do
      reversed_places = described_class.new(center_1, city_1, city_2, center_1).reverse
  ...
  describe "doesn't equal" do
    let(:places) { described_class.new(center_1, city_1, city_2, center_1) }
    let(:diferent_city) { MultipleVehicleRouting::City.new(name: 'c', coordinates: MultipleVehicleRouting::Coordinates.new(0, -2), capacity: 6) }
    ...
    it 'another places that begins with different center' do
      another_places = described_class.new(center_2, city_1, city_2, center_1)
      expect(places).not_to(eq(another_places))
    end

    it 'another place that ends with different center' do
      another_places = described_class.new(center_1, city_1, city_2, center_2)
      expect(places).not_to(eq(another_places))
    end

    it 'another place that one different city' do
      another_places = described_class.new(center_1, city_1, diferent_city, center_2)
      expect(places).not_to(eq(another_places))
    end

    it 'another place that one extra city' do
      another_places = described_class.new(center_1, city_1, city_2, diferent_city, center_2)
      expect(places).not_to(eq(another_places))
    end

    it 'another place that one less city' do
      another_places = described_class.new(center_1, city_1, center_2)
      expect(places).not_to(eq(another_places))
    end

    it 'another places with reverse order' do
      another_places = described_class.new(center_1, city_2, city_1, center_1)
      expect(places).not_to(eq(another_places))
    end
  end
  ...
  describe 'hash' do
    it 'is same hash for when equal' do
      expect(described_class.new(center_1, city_1, center_1).hash).to(eq(described_class.new(center_1, city_1, center_1).hash))
    end
  end

  describe 'round trip distance' do
    it 'is the sum of distance between two consecutive places' do
      expect(described_class.new(center_1, city_1, center_1).round_trip_distance).to(eq(4.0))
      expect(described_class.new(center_2, city_1, city_2, center_2).round_trip_distance).to(eq(6.0))
    end
  end

  describe 'size' do
    it 'is the number of places present' do
      expect(described_class.new(center_1, city_1, city_2)).to(have(3).entries)
    end
  end

  describe 'interchange positions' do
    it 'exchanges two places' do
      places = described_class.new(center_1, city_1, city_2)
  ...
  describe 'equivalent positions' do
    it 'is true when the places refered by positions are cities' do
      expect(described_class.new(center_1, city_1, city_2)).to(be_equivalent_positions(1, 2))
    end

    it 'is true when the places refered by positions are centers' do
      expect(described_class.new(center_1, city_1, center_2, city_2)).to(be_equivalent_positions(0, 2))
    end

    it 'is false when the places refered by positions are a city and a center' do
      expect(described_class.new(center_1, city_1, city_2)).not_to(be_equivalent_positions(0, 2))
    end
  end
  ...
  describe 'interchange positions on equivalence' do
    ...
    it 'exchanges two centers' do
      places = described_class.new(center_1, city_1, center_2)
  ...
  describe 'replicate first place to end' do
    it 'copies over copies over the beginning place to the end' do
      places = described_class.new(center_1, city_1, city_2)
      expect(places.replicate_first_place_to_end).to(eq(described_class.new(center_1, city_1, city_2, center_1)))
  ...
  describe 'split by leading center' do
    it 'gives an array of the cities separated by its leading center' do
      sub_places = described_class.new(center_1, city_1, city_2, center_2, city_3).split_by_leading_center
    ...
    it "doesn't include cities without a leading center" do
      sub_places = described_class.new(city_1, city_2, center_2, city_3).split_by_leading_center
